...
function sellTickets(queue) {
  return !queue.slice(0, -1).reduce((acc, value) => acc - value, queue.at(-1));
}
...
function sortCitiesArray(arr) {
  function getSortedValues(targetArr, key) {
    return Object.values(
      Object.fromEntries(
        Object.entries(Object.groupBy(targetArr, (item) => item[key])).sort()
      )
    );
  }
  ...
  const groupArr = getSortedValues(arr, keys[0]);

  groupArr.forEach((array) => {
    const sortedEntries = Object.values(
      Object.fromEntries(
        Object.entries(Object.groupBy(array, (item) => item[keys[1]])).sort()
      )
    );
...
function group(array, keySelector, valueSelector) {
  ...
  const keys = Object.keys(Object.groupBy(array, keySelector));

  keys.forEach((key, i) => {
    const values = Object.values(Object.groupBy(array, keySelector))[i].map(
      valueSelector
    );
